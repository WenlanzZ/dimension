...
bench_params$runs <- foreach(i = seq_len(nrow(bench_params)), 
  ...
  foreach(s = seq_len(bench_params$num_sim[i]), 
    ...
    bench <- k %>% add_column(lad_est = test2$d, lad_time = stime2[[3]])
...
bench <- arrange(bench, n, p, d, sigma)
bench %>% print(n = Inf)
bench1 <- bench %>% filter(d == 3) %>% mutate(mse = map_dbl(runs, 
                        ~ mean((.x$dim_est - rep(3, length(.x$dim_est)))^2)),
         mae = map_dbl(runs,
                        ~ mean(abs((.x$dim_est - rep(3, length(.x$dim_est)))))),
         bias = map_dbl(runs, 
                             ~ mean(.x$dim_est - rep(3, length(.x$dim_est)))),
         mean_est = map_dbl(runs, ~ mean(.x$dim_est)),
         median_est = map_dbl(runs, ~ median(.x$dim_est)),
         accuracy = map_dbl(runs, ~ sum(.x$dim_est == 3)),
         plots = map(runs, est_hist))

bench2 <- bench %>% filter(d == 10) %>% mutate(mse = map_dbl(runs, 
                        ~ mean((.x$dim_est - rep(10, length(.x$dim_est)))^2)),
         mae = map_dbl(runs,
                        ~ mean(abs((.x$dim_est - rep(10, length(.x$dim_est)))))),
         bias = map_dbl(runs, 
                             ~ mean(.x$dim_est - rep(10, length(.x$dim_est)))),
         mean_est = map_dbl(runs, ~ mean(.x$dim_est)),
         median_est = map_dbl(runs, ~ median(.x$dim_est)),
         accuracy = map_dbl(runs, ~ sum(.x$dim_est == 10)),
         plots = map(runs, est_hist))
...
bench_all %>% filter(rank_estimator_type == "kmeans") %>% subset(select=c(1:4, 13, 10, 11, 12))
bench_all %>% filter(rank_estimator_type == "posterior") %>% subset(select=c(1:4, 13, 10, 11, 12))
bench_all %>% filter(rank_estimator_type == "ladle") %>% subset(select=c(1:4, 13, 10, 11, 12))
bench_all %>% filter(rank_estimator_type == "double_post_cor") %>% subset(select=c(1:4, 13, 10, 11, 12))
bench_all %>% filter(rank_estimator_type %in% c("double_post", "posterior", "kmeans")) %>% subset(select=c(1:4, 7, 13, 10, 11, 12))%>% print(n = Inf)
bench_all %>% filter(rank_estimator_type == "posterior_cor") %>% subset(select=c(1:4, 13, 10, 11, 8, 12))
bench_all %>% trelliscope(name = "Dimension Estimation", panel_col = "plots")
