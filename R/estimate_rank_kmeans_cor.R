...
#' @export
estimate_rank_kmeans_cor <- function(s, verbose, ...) {
  UseMethod("estimate_rank_kmeans_cor", s)
}

#' @export
estimate_rank_kmeans_cor.default <- function(s, verbose, ...) {
  stop("Don't know how to estimate the rank for an object of type ",
       paste(class(s), collapse = " "), ".")
}

estimate_rank_kmeans_cor.subspace <- function(s, verbose = TRUE, ...) {
  # -----------------------
  ...
  prob_irl <- prob_irl[seq_len(cor_rnk)]
  prob_irl_diff <- abs(diff(prob_irl))
