           ...
           double* mu, double* psi, double* lt)
{
  ...
  for(l = 1; l <= *L; ++l) 
  {
    ...
    lmom[2 * l] = lmom[2 * l - 1] + 
      log((2 * l - 1) * s2 * exp(lmom[ 2 * l - 2] - lmom[ 2 * l - 1]) + mud );
  
    lmom[2 * l + 1] =  lmom[2 * l] + log(2 * l * s2 * 
      exp(lmom[ 2 * l - 1] - lmom[2 * l]) + mud );

    lt[l] = lr[l] + lmom[2 * l] + l * (lE2 + 2.0 * lphi) - l * log(4) -
      lgamma(*m / 2.0 + l) - lgamma(l+1);
  }
}
          ...
          double* mu, double* psi, double* lt ,double* lmom, double* lr)
{
  ...
  for (l = 1 ; l <= *L ; ++l) 
  {
    ...
    lmom[2 * l + 1] = lmom[2 * l] + 
      log((2 * l) * s2 * exp(lmom[2 * l - 1] - lmom[2 * l]) + mud);
