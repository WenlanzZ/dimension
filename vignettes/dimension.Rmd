...
We followed the suggested pipeline in the `Seruat` package (see [details](https://satijalab.org/seurat/v3.1/pbmc3k_tutorial.html)) and determined the signal dimension of 5 using the `JackStraw()` function. We then applied the louvain clustering method implemented `Seruat` for PCs from 1:50, 9:50, 1:5 and 1:8 (see [details](https://rpubs.com/WenlanzZ/578132)). The UMAP for each of plot confirmed that major features for cell type clustering depend on dimension 1 to 8 and there is no new cluster formed from dimension 9 to 50. Clustering from `dimension` estimated signal subspace identified two additional cluster that is masked in the whole space, while clustering from `JackStraw()` estimated signal subsapce failed to seperate cluster 0. 
...
Comparing to the original identification published by [Adams et  al.(2019)](https://www.biorxiv.org/content/10.1101/759902v1.full.pdf)), we cluster T cells together with NK cells and Basal cells with Club cells for dimension 1:8. Given that we used a subset of the original data and the fact that these cells regulate and differentiate into each other, the cell type identification result makes sense biologically. By contrast, if we include noisy PCs all together from 1 to 50 will misclassify T cells, Fibroblast, Basal cells, and Club cells together. Similarly, if we truncate signals as suggested by Jackstraw from 1 to 5, we will miss the important PCs that seperate Basal & Club cells from AT1 cells.
