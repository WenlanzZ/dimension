...
Output from `modified_legacyplot()` shows the posterior means and posterior probabilities of change points in a given sequence. The first plot, which uses the scaled eigenvalues as the argument, shows that there are highest posterior probabilities of change points at dimension 1, 2, 3, 4 and smaller posterior probabilities of change points at 7, 8, and 16. The second plot, which uses the posterior probabilities of change points as the argument, shows that there are highest posterior probabilities of probabilities of change points ("candidates"") at 4, 6, 8, 10, 11, and 13 to 18. The `dimension()` function looks at the second plot from right to left for "candidates" and then looks at the first plot from right to left for the maximum posterior probability of change point (4 in this example). In the first plot, if the posterior probability of change point at any of the "candidates" is not smaller than 90% of the posterior probability of change point at 4, then largest dimension that passed the threshold ("p" in `dimension` function) will be the estimated changepoint. Otherwise, the dimension corresponding to the maximum posterior probability in the first plot (4 in this example) will be the estimated changepoint.
...
rownames(marker4[order(marker4$avg_logFC, decreasing = T),][1:20,])
rownames(marker5[order(marker5$avg_logFC, decreasing = T),][1:20,])
```

We applied the clustering method implemented in the `Seruat` package (see [details](https://satijalab.org/seurat/v3.1/pbmc3k_tutorial.html)) for PCs from 1:50, 9:50, 1:5 and 1:8. The umap for each of plot confirmed that major features for cell type clustering depend on dimension 1 to 8 and there is no new cluster formed from dimension 9 to 50. Clustering from denoised signal subspace identified two additional cluster that is masked in the whole space. To validate the clustering results, we use the `FindMarkers()` function to identify marker genes in each cluster via differential expression and further match the clustering results to known cell types with identified marker genes. 
...
| 1          |  FTL, APOC1, APOE    |  Macrophage & Macrophage_Alveolar |
| 2          |  AGER, HOPX          |  AT1                              |
| 3          |  SCGB1A1, KRT17      |  Club cells & Basal cells         |
| 4          |  ABCG1, GLDN         |  Macrophage_Alveolar              |
| 5          |  APOD, DCN, GPX3     |  Fibroblast                       |
"
cat(tabl)
```
```{r fig1, fig.height = 5, fig.width = 9}
# Mark cell types with our identification
new.cluster.ids <- c("NK cells", "   Mac & Mac_Alv", "AT1 ", "Club & Basal", "Mac_Alv", "Fibroblast")
names(new.cluster.ids) <- levels(lung)
lung <- RenameIdents(lung, new.cluster.ids)
p5 <- DimPlot(lung, reduction = "umap", label = TRUE, pt.size = 0.5, label.size = 6) + NoLegend() + ggtitle("a) Cell Type Identification")

# Mark cell types with publication identification
subset_origin_ident <- read.table("/Users/wz262/Projects/dimension/data-raw/subset_origin_ident.txt", sep="\t", header=TRUE)
library(plyr)
subset_origin_ident$cell.type.ident <- revalue(subset_origin_ident$cell.type.ident, c("Macrophage_Alveolar"="Mac_Alv"))
lung@meta.data$orig_label <- subset_origin_ident$cell.type.ident
p6 <- DimPlot(lung, reduction = "umap",label = TRUE, group.by = "orig_label", label.size = 6) + NoLegend() + ggtitle("b) Public Cell Types")
...
Comparing to the original identification published by [Adams et  al.(2019)](https://www.biorxiv.org/content/10.1101/759902v1.full.pdf)), we cluster T cells together with NK cells and Basal cells with Club cells for dimension 1:6. Given that we used a subset of the original data and the fact that these cells regulate and differiate into each other, the cell type identification result makes sense biologically. By contrast, if we include noisy PCs all together from 1 to 50 will misclassify T cells, Fibroblast, Basal cells, and Club cells together. Similarly, if we truncate signals as suggested by Jackstraw from 1 to 5, we will miss the important PCs that seperate Basal & Club cells from AT1 cells. 
