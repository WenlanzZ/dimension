...
```{r}
lung <- RunUMAP(lung, method = 'umap-learn', reduction = "pca", dims = 1:50, verbose = FALSE)
...
new.cluster.ids <- c("NK cells", "Macrophage & Mac_Alv", "AT1 ", "Club cells & Basal cells", "Mac_Alv", "Fibroblast")
names(new.cluster.ids) <- levels(lung)
lung <- RenameIdents(lung, new.cluster.ids)
p5 <- DimPlot(lung, reduction = "umap", label = TRUE, pt.size = 0.5, label.size = 5) + NoLegend() + ggtitle("a) Cell Type Identification")
...
p6 <- DimPlot(lung, reduction = "umap",label = TRUE, group.by = "orig_label", label.size = 5) + NoLegend() + ggtitle("b) Public Cell Types")

# pdf("/Users/wz262/Projects/dimension/vignettes/cell_type.pdf", width = 15, height = 6)
